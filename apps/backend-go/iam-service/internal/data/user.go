...
func (r *userRepo) Create(ctx context.Context, user *biz.User) (*biz.User, error) {
	...
	err := r.data.DB(ctx).ExecInTransaction(ctx, tenantID, func(ctx context.Context, tx pgx.Tx) error {
		return tx.QueryRow(ctx,
			`INSERT INTO users (external_id, username, email, display_name) VALUES ($1, $2, $3, $4) RETURNING id, created_at, updated_at`,
			user.ExternalID, user.Username, user.Email, user.DisplayName,
...
func (r *userRepo) GetByID(ctx context.Context, id string) (*biz.User, error) {
	...
	err := r.data.DB(ctx).ExecInTransaction(ctx, tenantID, func(ctx context.Context, tx pgx.Tx) error {
		err := tx.QueryRow(ctx,
			`SELECT id, external_id, username, email, display_name, created_at, updated_at FROM users WHERE id = $1 AND deleted_at IS NULL`, id,
		).Scan(&u.ID, &u.ExternalID, &u.Username, &u.Email, &u.DisplayName, &u.CreatedAt, &u.UpdatedAt)
...
func (r *userRepo) GetByExternalID(ctx context.Context, externalID string) (*biz.User, error) {
	...
	err := r.data.DB(ctx).ExecInTransaction(ctx, tenantID, func(ctx context.Context, tx pgx.Tx) error {
		err := tx.QueryRow(ctx,
			`SELECT id, external_id, username, email, display_name, created_at, updated_at FROM users WHERE external_id = $1 AND deleted_at IS NULL`, externalID,
		).Scan(&u.ID, &u.ExternalID, &u.Username, &u.Email, &u.DisplayName, &u.CreatedAt, &u.UpdatedAt)
		if err == pgx.ErrNoRows {
			return nil
		}
...
func (r *userRepo) Update(ctx context.Context, user *biz.User) (*biz.User, error) {
	...
	err := r.data.DB(ctx).ExecInTransaction(ctx, tenantID, func(ctx context.Context, tx pgx.Tx) error {
		return tx.QueryRow(ctx,
			`UPDATE users SET username = $2, email = $3, display_name = $4, updated_at = now() WHERE id = $1 RETURNING updated_at`,
...
func (r *userRepo) ListByTenant(ctx context.Context, tenantID string, pageSize int, cursor string) ([]*biz.User, string, error) {
	...
	err := r.data.DB(ctx).ExecInTransaction(ctx, tenantID, func(ctx context.Context, tx pgx.Tx) error {
		rows, err := tx.Query(ctx,
			`SELECT u.id, u.external_id, u.username, u.email, u.display_name, u.created_at, u.updated_at
		...
		for rows.Next() {
			u := &biz.User{}
			if err := rows.Scan(&u.ID, &u.ExternalID, &u.Username, &u.Email, &u.DisplayName, &u.CreatedAt, &u.UpdatedAt); err != nil {
				return err
			}
